  ...
  <section class="pb-4">
    ...
    <section class="pl-2 pb-0">
      <table class="bg-gray-100 shadow-md">
        <tr
          class="bg-gray-100"
          in:receive="{{key:headerKey(reportName)}}"
          out:send="{{key:headerKey(reportName)}}"
        >
          <th class="px-4 pt-1 w-40 text-left font-medium text-gray-700">
            Source
          </th>
          {#each Object.keys(groupedByReportName[reportName][0].metrics) as
          metricName}
          <th class="px-4 pt-1 w-40 text-left font-normal text-gray-600">
            {metricName}
          </th>
          {/each}
        </tr>
        ...
        <tr
          ...
          in:receive="{{key:rowKey(report)}}"
          out:send="{{key:rowKey(report)}}"
        >
          <td class="px-4 pb-1 w-40 text-gray-800 font-hairline">
            {report.sourceName}
          </td>
          {#each Object.entries(report.metrics) as metric}
          <td class="px-4 pb-1 w-40 text-gray-500 hover:text-yellow-800">
            {metric[1]}
          </td>
          {/each}
        </tr>
